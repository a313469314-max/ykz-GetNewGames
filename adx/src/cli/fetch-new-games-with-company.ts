...
const COMPANY_REPORT_PREFIX = "dataeye_new_games_company";
...
async function fetchCompanyNamesWithInteractiveRecovery(productIds: string[]): ReturnType<typeof fetchProductCompanyNames> {
  try {
    ...
    console.log("补齐公司名称时检测到 DataEye 登录态异常，正在打开浏览器登录页。");
    console.log("请完成验证码和登录，登录成功后程序会自动继续补齐公司名称。");
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const reportText = buildDailyReportText(targetDay.statDate, rowsWithCompany, {
      ...
      title: `DataEye 新品公司日报 (${targetDay.statDate})`
    });

    await writeFile(jsonPath, `${JSON.stringify(rowsWithCompany, null, 2)}\n`, "utf8");
    await writeFile(csvPath, `${UTF8_BOM}${toCsv(rowsWithCompany, { includeCompanyName: true })}`, "utf8");
    const reportPath = await writeDailyReport(targetDay.statDate, reportText, {
      filenamePrefix: COMPANY_REPORT_PREFIX
    });
